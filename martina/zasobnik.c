...
int zasobnik_init (TZasobnik *zas){
    if ((zas->array = malloc(REALLOC_SOUSTO*sizeof(int)))==NULL){    // k malloc musi byt free !!!
        return ERR_INTERNI;
    }
...
int zasobnik_push (TZasobnik *zas, int hodn){
    if(zas->top == zas->velikost-1) {
        if((zas->array = realloc(zas->array, ((zas->velikost + REALLOC_SOUSTO) * sizeof(int)))) == NULL) {    // k malloc musi byt free !!!
            return ERR_INTERNI;
        }
        zas->velikost += REALLOC_SOUSTO;
        printf(">>>Probehla realokace<<<\n");
        printf(">>>>>>>>>>>>>Pro me: sizeof(int) je %d, REALLOC_SOUSTO JE %d, zas->velikost je %d\n", sizeof(int), REALLOC_SOUSTO, zas->velikost);
    }
    zas->top++;
...
*/
int zasobnik_pristup (TZasobnik *zas, int * hodn, int posun){
    if (zas->top < 0-posun){
        return ERR_INTERNI;
    }
